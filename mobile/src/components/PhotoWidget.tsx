import React from 'react'
import { View, Text, Image, TouchableOpacity, StyleSheet } from 'react-native'
import * as ImagePicker from 'expo-image-picker'
import { colors, radius, font, space } from '../theme'

interface Props {
  uri: string | null
  onChange: (uri: string | null, base64?: string | null) => void
  fallbackIcon?: string
  size?: number
}

export default function PhotoWidget({ uri, onChange, fallbackIcon = '👤', size = 100 }: Props) {
  const pick = async (useCamera: boolean) => {
    const opts: ImagePicker.ImagePickerOptions = {
      quality: 0.6,
      allowsEditing: true,
      aspect: [1, 1],
      base64: true,   // get base64 directly — avoids content:// URI issues on Android
    }
    const r = useCamera
      ? await ImagePicker.launchCameraAsync(opts)
      : await ImagePicker.launchImageLibraryAsync(opts)
    if (!r.canceled && r.assets[0]) {
      onChange(r.assets[0].uri, r.assets[0].base64 ?? null)
    }
  }

  return (
    <View style={s.wrap}>
      <TouchableOpacity style={[s.thumb, { width: size, height: size }]} onPress={() => pick(true)}>
        {uri ? <Image source={{ uri }} style={s.img} /> : <Text style={{ fontSize: size * 0.38 }}>{fallbackIcon}</Text>}
      </TouchableOpacity>
      <View style={s.row}>
        <TouchableOpacity style={s.btn} onPress={() => pick(true)}><Text style={s.btnTxt}>📷</Text></TouchableOpacity>
        <TouchableOpacity style={s.btn} onPress={() => pick(false)}><Text style={s.btnTxt}>🗂</Text></TouchableOpacity>
        {uri && <TouchableOpacity style={[s.btn, { borderColor: colors.red }]} onPress={() => onChange(null, null)}><Text style={[s.btnTxt, { color: colors.red }]}>✕</Text></TouchableOpacity>}
      </View>
    </View>
  )
}

const s = StyleSheet.create({
  wrap:   { alignItems: 'center', gap: space.xs },
  thumb:  { borderRadius: radius.md, borderWidth: 2, borderColor: colors.borderHi, backgroundColor: colors.elevated, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
  img:    { width: '100%', height: '100%' },
  row:    { flexDirection: 'row', gap: 6 },
  btn:    { paddingVertical: 5, paddingHorizontal: 10, borderRadius: radius.xs, borderWidth: 1, borderColor: colors.borderHi, backgroundColor: colors.elevated },
  btnTxt: { fontSize: font.md, color: colors.textSec },
})
